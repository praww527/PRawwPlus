...
import {
  LogOut, CreditCard, Zap, Receipt, ChevronRight,
  ShieldCheck, AlertCircle, CheckCircle2, Loader2, User
} from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 animate-pulse">
        <div className="h-28 rounded-3xl glass" />
        <div className="h-40 rounded-3xl glass" />
        <div className="h-32 rounded-3xl glass" />
      </div>
    );
  }
    ...
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-4">
      ...
      <div className="glass rounded-3xl p-5 border border-white/10 flex items-center gap-4">
        <div className="w-14 h-14 rounded-2xl bg-primary/20 border border-primary/30 flex items-center justify-center shrink-0">
          {authUser?.profileImage ? (
            <img src={authUser.profileImage} alt="" className="w-full h-full rounded-2xl object-cover" />
          ) : (
            <User className="h-7 w-7 text-primary" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <p className="font-bold text-white text-lg truncate">{user?.name || user?.username}</p>
          <p className="text-sm text-white/40 truncate">@{user?.username}</p>
          {user?.isAdmin && (
            <span className="inline-flex items-center gap-1 mt-1 text-[10px] font-bold tracking-wider px-2 py-0.5 rounded-full bg-primary/15 border border-primary/25 text-primary">
        ...
        <button
          onClick={logout}
          className="p-2.5 rounded-xl glass border border-white/10 text-white/50 hover:text-red-400 hover:border-red-500/20 transition-colors"
        >
      ...
      <div className="glass rounded-3xl p-5 border border-white/10 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs text-white/40 uppercase tracking-widest font-semibold mb-1">Credit Balance</p>
            <p className={cn(
              "text-4xl font-display font-bold",
              (user?.creditBalance ?? 0) > 10 ? "text-white" :
              (user?.creditBalance ?? 0) > 0 ? "text-amber-400" : "text-red-400"
            )}>
        ...
        <div className="border-t border-white/8 pt-4 flex flex-col gap-3">
          ...
          <button
            ...
            className="flex items-center justify-between w-full px-4 py-3.5 rounded-2xl bg-primary/15 border border-primary/25 hover:bg-primary/20 transition-all active:scale-[0.98] group"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40">R100/month · R20 credit included</p>
              </div>
            </div>
            {subscribing
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-white/40">Add more calling credit instantly</p>
                </div>
              </div>
              <ChevronRight className="h-4 w-4 text-white/30 group-hover:text-white transition-colors" />
            </button>
          ) : (
            <div className="px-4 py-3.5 rounded-2xl glass border border-amber-500/20 space-y-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl glass border border-white/10 text-white/60 text-sm hover:text-white transition-colors"
                >
                  Cancel
                </button>
                <button
                  onClick={handleTopup}
                  disabled={toppingUp}
                  className="flex-1 py-2.5 rounded-xl bg-amber-500/20 border border-amber-500/30 text-amber-400 text-sm font-semibold hover:bg-amber-500/30 transition-all"
                >
                  {toppingUp ? <Loader2 className="h-4 w-4 animate-spin mx-auto" /> : `Pay R${topupAmount}`}
                </button>
              </div>
            </div>
          )}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="glass rounded-2xl p-4 border border-white/10 text-center">
          <p className="text-2xl font-bold text-white">R{(user?.totalCreditUsed ?? 0).toFixed(2)}</p>
            ...
            {recentPayments.map((p) => (
              <div key={p.id} className="flex items-center justify-between px-5 py-3.5">
                <div>
                  ...
                  <p className="text-xs text-white/40 mt-0.5">{format(new Date(p.createdAt), "MMM d, yyyy")}</p>
                </div>
