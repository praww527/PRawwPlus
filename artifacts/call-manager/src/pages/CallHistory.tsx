      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-[70px] rounded-2xl glass animate-pulse" />
          ))}
        </div>
      ) : data?.calls.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <p className="text-white/40 text-sm">No calls yet. Start dialing!</p>
        </div>
      ) : (
        <div className="space-y-2.5">
          {data?.calls.map((call) => (
            <div
              key={call.id}
              className="flex items-center gap-3 p-3.5 rounded-2xl glass border border-white/8 hover:border-white/15 transition-all"
            >
              {/* Status icon */}
              <div
                className={cn(
        ...
        <div className="flex items-center justify-between pt-2 pb-2">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="w-9 h-9 rounded-full glass border border-white/10 hover:border-white/20 disabled:opacity-30 transition-all flex items-center justify-center active:scale-90"
            >
