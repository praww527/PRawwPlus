...
class MetricsStore {
  ...
  // ── Latency samples ────────────────────────────────────────────────────────
  // Ring-buffer of call setup latency measurements (ms).
  ...
  recordCallSetupLatency(ms: number): void {
    ...
    this.latencySamples.push({ value: ms, ts: Date.now() });
  }

  /** Return p50/p95/p99 of call setup latency over the last N samples. */
  callSetupLatencyPercentiles(): { p50: number; p95: number; p99: number; count: number } {
    const values = this.latencySamples.map((s) => s.value).sort((a, b) => a - b);
